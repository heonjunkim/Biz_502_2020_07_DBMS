-- 여기는 User1 화면입니다
-- tbl_iolist에는 상품명, 거래처CEO 칼럼의 데이터가 일반 문자열 형태로 저장되어 있다.
-- 이 데이터는 같은 칼럼에 중복된 데이터가 있어서 데이터 관리 측면에서 상당히 불편한 상황이다.
-- 만약 어떤 상품의 상품명이 변경이 필요한 경우 
-- 상품명을 UPDATE 하여야 하는데, 2개이상의 레코드를 대상으로 UPDATE 과정이 필요하다.
-- 2개이상의 레코드를 UPDATE 수행하는 것은 데이터의 무결성을 해칠 수 있는 수행이 된다.
-- 이러한 문제를 방지하기 위해 다음과 같은 정규화를 실행한다.

-- 상품명 정보를 별도의 테이블로 분리하고, 상품정보에 상품코드를 부여한 후
-- tbl_iolist와 연동하는 방식으로 정규화를 실행한다.

-- tbl_iolist 로부터 상품명 리스트를 추출하자
...
-- 위 Query를 실행하여 P.pname 항목에 (null) 값이 있다면 상품정보 테이블이  잘못만들어진 것이다.         
-- 상품정보 테이블을 삭제하고 과정을 다시 수행해야 한다.

-- 쿼리 결과중에 P.pname 항목의 값이 null인 리스트만 보여라
-- 쿼리결과가 정상이라면 리스트는 한개도 없어야 한다.
SELECT IO.io_pname, P.p_name 
FROM  tbl_iolist IO
    LEFT JOIN tbl_product P
        ON IO.io_pname = P.p_name
WHERE P.pname IS NULL;       -- 해당 칼럼 값이 NULL이 아니냐 ? : IS NOT NULL 데이터의 빈값은 절대 맘대로 수정하면 안된다
...
--ALTER TABLE을 할때
-- 이미 많은 데이터가 INSERT가 되어 있는 상태에서 칼럼을 추가하면
...
-- 칼럼의 type 변경하기 주의사항
-- 칼럼의 type을 변경할때 
--      문자열 <==> 숫자 처럼 type이 완전히 다른 경우는 오류가 발생하거나
--               데이터를 모두 잃을 수 있다,.
--      CHAR <==>  (n)VARCHAR2    
...
--      데이터가 갈리거나, 문자열이 알수없는 값으로 변형되는 경우가 발생할 수 있다.

-- 칼럼의 이름 변경하기
-- io_pcode 이름을 io_pcode1으로
ALTER TABLE tbl_iolist RENAME COLUMN  io_pcode TO io_pcode1;


-- 상품정보에서 매입매출장 각 레코드의 상품명과 일치하는 상품코드를 찾아서
-- 매입매출장의 상품코드(io_pcode) 칼럼에 UPDATE 해주기
...
--  4.  sub Query는 전달받은 io_pname값을 tbl_product 테이블에서 조회를 한다.
...
-- 이 데이터에서 거래처명으로 조회를 하면 출력되는 레코드(row)가 2개이상인 경우가 발생한다.
-- 따라서 이 쿼리를 실행하면 ORA-01427: single-row subquery returns more than one row 오류가 발생한다.
-- 이 쿼리는 거래처명과 CEO 값을 동시에 제한하여 1개의 raw값만 sub Query에서 만들어 지도록 해야 한다.
UPDATE tbl_iolist IO
SET io_bcode =
(
    SELECT b_code
...
-- tbl_iolist에서 io_dname, io_dceo 칼럼은 필요가 없으므로 제거한다.
...
SELECT io_seq, io_date, 
        io_bcode, b_name, b_ceo, b_tel, io_pcode, p_name, p_iprice, p_oprice,
io_inout,

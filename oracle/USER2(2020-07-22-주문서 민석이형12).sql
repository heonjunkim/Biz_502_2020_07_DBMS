...
INSERT INTO tbl_order(o_seq, o_date, o_num, o_cnum, o_pcode) 
        VALUES (seq_order.NEXTVAL, '2020-07-22', 'O00001', 'C0031', 'P00001');
INSERT INTO tbl_order(o_seq, o_date, o_num, o_cnum, o_pcode) 
        VALUES (seq_order.NEXTVAL, '2020-07-22', 'O00001', 'C0031', 'P00002');
INSERT INTO tbl_order(o_seq, o_date, o_num, o_cnum, o_pcode)
        VALUES (seq_order.NEXTVAL, '2020-07-22', 'O00001', 'C0031', 'P00003');
    
SELECT o_num, o_cnum, o_pcode FROM tbl_order;

-- 지금 tbl_order 테이블에 위와 같은 데이터가 있을 때
-- O00001     C0031     P00001 이러한 데이터를 추가한다면 아무런 제약없이 값이 추가되어버릴 것이다.
...
-- duplicate keys found : PK, UNIQUE로 설정된 칼럼에 값을 추가하거나, 이미 중복된 값이 있는데 PK, UNIQUE를 설정하려 할때 발생!

-- UNIQUE 제약조건을 추가하기 위해 마지막에 추가된 데이터를 제거하려고 다음과 같은 SQL을 실행한다.
-- 이 SQL을 실행하면, 삭제되어서는 안되는 중요한 주문정보 1건이 같이 삭제되어버릴 것이다.
...
-- 후보키 중에 단일칼럼으로 PK를 설정할 수 없는 상황이 발생을 하면 복수의 칼럼으로 PK를 설정하는데

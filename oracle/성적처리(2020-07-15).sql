-- Grade User로 접속한 화면
-- CONSTRAINTS : 계약조건, 제약사항, Valid Option
...
-- UNIQUE : 중복되지 않는 값만 유효, 해당 칼럼에 추가하고자 하는 데이터가
--       이미 저장되어 있으면 추가로 저장할수 없다.

-- CHECK : 어떤 값이 해당 범위에서 유효한 범위만 추가가능
-- 문자열 칼럼의 Type
-- CHAR(자릿수) : 저장하는 데이터의 길이가 모두 일정할 경우 사용하는 Type
--      저장하는 Data 자릿수가 같지 않으면 부족한 부분을 공백으로 채우는데
--      DB에 따라 문자열의 앞, 뒤에 공백을 추가하여 간혹 조회를 할때 
--      조회가 잘 안될수도 있다.
-- VARCHAR2(자릿수) : 저장하는 데이터의 길이가 일정하지 않을 경우 사용하는 Type
--  저장하는 Data가 설정한 자릿수보다 적으면 칼럼의 실제 크기를 줄여서 저장한다.
--  저장하는데 CHAR보다 다소 지연되지만 지금은 큰 문제가 없다.
--  저장할 데이터를 잘 분석하여 최대 자릿수를 데이터의 최대 크기만큼 지정한다.
--  저장할 데이터의 길이가 자릿수보다 크면 저장되지 않는다.
-- nVARCHAR2(자릿수) : VARCHAR2(자리수)와 성질이 같은 Type
--      영문자 전용이 아닌 한글, 한자 등 알파벳 이외의 문자를 저장할때 사용
--      UNICODE 문자열을 위해 특별히 마련된 Type
--      영문자1글자와 한글1글자 값인 자리수로 취급한다.
--      키보드에 기본적으로 있는 문자가 아닌 문자를 저장하는 경우는
--      무조건 nVARCHAR()를 사용하자

-- NUMBER(자릿수, 소수점) : 오라클에서는 숫자형일 경우 별도의 Type을 명시하지 않고
-- NUMBER(자릿수, 소수점) 형식으로 사용을한다.
-- NUMBER(자릿수) : 정수형을 표현
...
CREATE TABLE tbl_student (
        ...
        st_grade NUMBER(1) CHECK(st_grade > 0 AND st_grade <= 4),--1이상 4이하	
        st_tel	nVARCHAR2(20),		
...
-- INSERT를 수행후 자주 발생하는 오류들
-- ORA-00001: unique constraint (GRADE.SYS_C007009) violated : PK나, UNIQUE
-- 로 설정된 칼럼에 이미 저장된 값을 또 저장하려고 할때

-- INSERT를 수행한후 데이터확인
--INSERT후 데이터를 확인할때는 모든 데이터를 조회하는 것보다
-- 현재입력한 데이터의 PK를 기준으로 조회하는 것이 정확도가 높다.
SELECT *
FROM tbl_student
WHERE st_num = '10001';
...
-- 예) 나열된 칼럼개수 6개인데, Values 데이터는 6개미만으로 설정된 경우
...
-- INSERT를 수행후 자주 발생하는 오류들
-- ORA-01400: cannot insert NULL into ("GRADE"."TBL_STUDENT"."ST_NUM") :
...
--      내부에서 문자열 > 숫자 형식으로 변환하여 저장한다. : 자동형변환
--      그런데 문자열 > 숫자 형식으로 자동형변환을 하는데 숫자로 변환할수 없는
...
-- INSERT를 수행후 자주 발생하는 오류들
-- ORA-02290: check constraint (GRADE.SYS_C007008) violated :

...
-- O0002        P0010
------------------------------------------
-- 위와 같은 형식의 테이블을 만들고 데이터를 분리한 다음
...
-- 테이블을 만들고 보니, 상품코드에 해당하는 상품명과 상품단가 판매가격이 없어서 보기가 불편하다.
-- 임의로 상품명, 판매가격 칼럼을 추가하고 데이터를 새롭게 만들었다.

-- 주문번호, 상품코드, 상품명,판매가격, 실제판매가격 칼럼을 가진 데이터 테이블이 되었다.
-- 이 테이블에 PK키를 설정하려고 후보키를 찾아봤더니 단독칼럼으로는 PK로 만들수가 없다.
-- 그래서 어쩔 수 없이(주문번호, 상품코드)2개의 키를 복합키로 하여 PK를 설정했다.

-- 데이터를 검증작업을 수행한다.
-- 실제 판매가격 = f(주문번호,상품코드)와 같은 함수가 있다면
-- 주문번호와 상품코드를 매개변수로 전달하여 결과로 실제판매가격을 추출할 수 있게 되었다.

-- 그런데, 상품명과 판매가격은 굳이 주문번호를 몰라도 상품코드만으로 해당 데이터를 추출할 수 있더라
-- 상품명 = F1(상품코드), 판매가격 = F2(상품코드) 형식의 함수만 있더라도 해당 데이터를 추출할 수 있더라
-- PK로 설정된 두개의 키값이 없어도 상품명과 판매가격을 조회할수 있다는 것인
-- 이테이블이 제2정규화 규칙에 어긋난다 라는 뜻이 된다.
...
-- 현재 테이블에는 주문번호, 상품코드, 실제판매가격 칼럼만 남겨놓는 것
--      이 상태를 제2정규화 완성이라고 한다.

-- 정규화의 가장 큰 목적
-- 추가이상 : 데이터를 추가할때 임의 칼럼 값이 정해지지 않아 null값이 추가되는 현상
-- 갱신이상 : 데이터를 변경할때 2개이상의 레코드가 변경되고, 이 때문에 원치않는 데이터에 변경이 수행되는 현상
-- 삭제이상 : 어떤 레코드를 삭제하면 그로인해 다른 정보를 조회할수 없게 되는 현상
--      상품정보 레코드를 제거하면, 그 상품의 판매데이터는 쓸모없는 데이터가 된다.
--      같은 테이블에 상품이름, 주매입처 와 같은 칼럼이 존재할때
--      해당 레코드를 삭제해 버리면 주매입처 정보가 같이 사라지는 현상
--    이러한 이상현상을 방지하는 목적

-- 데이터의 칼럼들에 필요없는 칼럼이 존재하여 해당칼럼 값이 null로 채워지는 것을 방지하는 목적
--      데이터베이스 성능, 효율과 관련된다.


-- **정보처리기사 DB의 정규화 출제문제
--    제1정규화,   제2정규화,   제3정규화,         BCNF,                  제4정규화,        제5정규화의 특징이 아닌것, 설명이 아닌것
-- 반복종속성 제거  부분종속성 제거 이행적함수 제거  후보키가 아닌 결정자 제거   다치종속제거            조인종속성 이용
-- 원자성을 성립
-- 반부이다조
-- 테이블을 조각조각분리한 테이블을 재결합

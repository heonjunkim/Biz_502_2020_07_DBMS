...
-- DEFAULT
-- 해당하는 칼럼에 데이터를 저장하지 않고 INSERT 수행하면
-- DEFAULT로 설정된 값을 지정한 것으로 이해하고 INSERT를 수행해라
...
-- 학생별 성적을 총점과 평균을 계산하여 리스트로 확인하기
...
-- SELECTION 적용하여 구현
-- 학번이 20001부터 20010까지 학생의 리스트만 보고 싶을때
SELECT * FROM tbl_score
WHERE sc_num BETWEEN '20001' AND '20010';


-- 학번이 20001~ 20010 인 학생들만 추출하여 과목별 총점을 계산하라
SELECT SUM(sc_kor) AS 국어총점,
        SUM(sc_eng)AS   영어총점, 
        SUM(sc_math)AS 수학총점,
        SUM(sc_music)AS 음악총점,
...
-- SUM() 함수를 이용하여 각 과목별총점을 계산하면서
-- 의미없는(DUMY) 칼럼을 '==과목총점==' 이라는 값으로 만들었다.
-- 통계, 집계 함수를 사용할때 통계, 집계함수로 묵이지 않은 칼럼은
-- 무조건 GROUP BY절에 칼럼을 나열해 주어야한다.
SELECT '==과목총점==',
        SUM(sc_kor) AS 국어,
        SUM(sc_eng)AS   영어, 
        SUM(sc_math)AS 수학,
...
WHERE sc_num BETWEEN '20001' AND '20010'
GROUP BY '==과목평균=='
-- 제목칼럼은 안넣으면 되지 않는다 ex? ==땡땡== 통계함수의 규칙 
-- 함수로묶여잇는 연산칼럼 GROUP BY 활용
-- 종료 세미콜론을 찍지 않는다!!!
-- 유니온으로 묶어서 3개의 계산식들을 통합시킴 
-- 셀렉트문이 여러개 나올때 묶어서 하나의 리스트로 보여줌
-- 데이터 개발자들의 고급언어 이런코드도 만들수잇다

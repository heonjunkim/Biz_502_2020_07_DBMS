...
-- tbl_student의 st_grade 칼럼에 저장된 값중에서 최대값(가장큰값),
-- 최소값(가장 작은값)을 보여달라
...
-- tbl_student 데이터 중에서 학과가 어떠한 학과들이 있는지 알고싶다.
-- 학과이름이 같은 이름들이 다수 존재하는데, 이들 이름을 1개씩만 추출하여
-- 리스트로 보여준다면 , 어떠한 학과가 있는지 알기 쉬울 거같읍니다.
-- st_dept 칼럼에 저장된 값들은 같은 값끼리 그룹을 지어라
...
-- st_name 칼럼에 저장된 값들이 중복된 값이 없어서 GROUP BY가 
-- 의미없는 경우도 있다.
SELECT st_name FROM tbl_student
GROUP BY st_name;

-- 데이터를 학과이름을 그룹으로 짓고
-- 각 학과에 소속된 학생들이 몇명씩인가? 를 알고싶다.
...
-- st_grade 칼럼으로 GROUP을 만들고
-- 각 그룹에 속하는 레코드가 몇개인가를 카운트하여 보여준다.
...
--  각 학과별로 소속된 학생들의 총 나이합계가 알고싶다.
SELECT st_dept , SUM(st_age) FROM tbl_student
GROUP BY st_dept;

-- 각 학과별로 소속된 평균 나이가 알고싶다.
SELECT st_dept , SUM(st_age) FROM tbl_student
GROUP BY st_dept, st_age;

-- 각 학과의 학생들 중에 나이가 가장많은 학생의 나이와
--      나이가 가장 적은 학생의 나이를 보여달라
--  단, 가장 나이가 많은 학생의 이름, 또는 나이가 가장적은 학생의 이름을 찾는데는
--                  이 SQL로는 어렵다!

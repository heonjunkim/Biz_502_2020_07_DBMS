...
--      보통Master Data Table이라고 한다.
--      (학생정보, 학과정보)
--      Maste Data 는 초기에 INSERT가 수행된 후에 업무가 진행동안 
--      가급적 데이터를 변경하거나, 삭제하는 일이 최소화 되어야 하는 데이터이다.
--      Master Data와 Realation을 하여 생성되는 여러데이터들의 무결성을 위해서 
--      Master Data는 변경을 최소화 하면서 유지 해야 한다.

-- DBMS의 스키마에 포함된 Table 중에 수시로 데이터가 추가,변경,삭제가필요한 
-- Table을 보통 work Data Table 이라고 한다.
--      (성적정보)
--      work Data는 수시로 데이터가 추가되고, 여러가지 연산을 수행하여
--  통계,집계등 보고서를 작성하는 기본 데이터가 된다.
...
UPDATE tbl_score -- 변경할테이블
SET sc_kor = 99 -- 변경할대상  = 값
WHERE sc_num = '20015' -- 조건(UPDATE에서 WHERE는 선택사항이나, 실무에서는 필수사항인식!)

UPDATE tbl_score
SET sc_kor = 99,sc_math = 90 -- 다수의 칼럼 값을 변경하고자 할때 칼럼 = 값, 칼럼 = 형식으로
WHERE sc_num = '20015'
SELECT * FROM tbl_score;
SELCECT * FROM tbl_score WHERE sc_num = '20015';
...
--    스토리제에 데이터 변경이 반영이 되기전에
--  ROLLBACK 명령을 수행하면 변경내용이 모두 제거(취소) 된다.
--  ROLLBACK명령을 잘못 수행하면, 정상적으로 변경(CUD) 이필요한 데이터마저
-- 변경이 취소되어 문제를 일으킬수 있다.

-- INSERT를 수행하고 난 직후에는 데이터의 변경이 물리적으로 반열될수 있도록
...
--20020 학번의 학생이 시험날 결석을하여 시험 응시를 하지 못했는데 입력이 됐다?
-- 이 학생의 성적데이터는 삭제되야한다!
-- 20020 학생이 정말 시험날 결석(?)한 학생인지 확인하는 절차가 필요하다
-- 20020 학생의 학생정보를 확인하고, 만약 이학생의 성적 정보가 등록되있다면
-- 삭제를 수행하자.
SELECT * FROM tbl_student WHERE st_num = '20020';

-- 아래 Query문을 실행햇을때
-- 학생정보는 보이는데, 성적정보 칼럼 값들이 모두 (null)로 나타내면
--  이 학생의 성적정보는 등록되지 않은 것이다.
--  따라서 삭제하는 과정이 필요하지 않다.
-- 학생정보와 함께 성적정보를 칼럼의 값들이 1개라도 (null)_이 아닌 것으로 나타나면
--      이 학생의 성적정보는 등록된 것이다.
--      따라서 학생의 성적정보를 삭제해야한다.
SELECT *
FROM tbl_student ST
...
-- 각 학생의 점수 평균을 구하고
-- 전체학생의 평균을 구하여 
-- 각 학생에 평균점수가 전체학생의 평균점수보다 높은 리스트를 조회하시오
...
-- 1. FROM절이 실행되어 tbl_score 테이블의 정보(칼럼정보)를 가져오기
-- 2.WHERE 절이 실행되어서 실제 가져올 데이터를 선별
-- 3 .GROUP BY 이 실행되어 중복된 데이터를 묶어서 하나로 만든다.
-- 4. SELECT 에 나열된 칼럼에 값을 채워놓고 ,
-- 5. SELECT에 정의된 수식을 연산, 결과를 보일준비
-- 6. ORDER BY는 모든쿼리가 실행되고 가장 마지막에 연산이 수행되어 정렬을 한다.

-- WHERE 결과 GROUP BY 절에서는 Alias 설정된 칼럼 이름을 사용할 수없다.
...
-- 추출된 반명칭이 '2006' 보다 작은 값을 담는 반만 추출
-- HAVING : 성질이 WHERE와 매우 비슷하다.
-- 하는일이 GROUP BY로 묶이거나, 통계함수 생성값을 대상으로
-- WHERE 연산을 수행하는 키워드
SELECT SUBSTR(sc_num,1,4) AS 반
FROM tbl_score
GROUP BY SUBSTR(sc_num,1,4)
...
-- 2000~ 2005 까지는 A그룹, 2006~2010 까지는 B 그룹일때
-- 반명이 '2003' 이하, A 그룹의 반들 평균 구하기

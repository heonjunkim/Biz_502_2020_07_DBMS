-- 여기는 sys 계정으로접속한화면입니다
-- TableSpace와 사용자 계정을 등록하고 사용자 계정에 권한을 부여

-- Oracle에서는 공식적으로 Data를 저장하기 앞서 TableSpace를 생성하라
-- TableSpace를 생성하지않고 Table 등을 만들고 Data를 저장하면
-- 오라클 System 폴더, System TableSpace에 데이터가 저장되어서
-- 보안적인 측면, 관리적인 측면에서 별로 좋지 않다 라고 한다.
...
-- 이러한 이유로 DBA(데이터베이스 관리자) 들도 실무경험이 많은 사람들이 다루는
-- 분야이다.
-- 그러다보니, 초급(시작하는 DBA)은 별로 다루지 않게 된다.
-- 보통 교제에선 TableSpace를 다루지 않는 경우가 많습니다.

-- TableSpace 생성에서 고려할 사항
-- 이름, 저장파일, 초기용량, 자동증가옵션 4가지만
-- 오라클 11gXe는 MaxSize를 지정하지 않아도 기본값으로 11G로 설정이 된다.
-- 따라서  11gXe를 사용할때는 최대 저장공간을 11G 사용할 수 있다.

CREATE TABLESPACE gradeTS
DATAFILE 'C:/bizwork/workspace/oracledata/gradeTS.dbf'

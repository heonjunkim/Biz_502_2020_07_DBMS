...
-- 00회사의 매입,매출을 관리 프로젝트를 수행하고 있다.
-- 이회사는 기존에 엑셀을 이용하여 거래처를 매입매출 관리를 수행해왔는데
-- 최근 App을 개발하여 데이터베이스화 하고 관리를 수행하려고 한다.
-- 엑셀 데이터를 받아서 DB로 보내기 위해 변환 작업을 수행하고 있다.

-- 엑셀 매입매출원장을 DB로 만들기 위해 매입, 매출을 분리하고, 날짜를 문자열화 했다.
-- 이 데이터를 DB에 저장하려고 봤더니, 이 데이터에 PK칼럼으로 지정할 만한 칼럼을 찾을수가 없다.
...
--      이러한 형태의 데이터는 각 칼럼의 NULL값이 존재하여, 다양한 연산을 수행할때 
--      문제를 일으킬 수 있다.
-- 변환된 데이터
--      수량, 단가 ,금액 형식으로 칼럼을 통합작업을 수행했다.
--      칼럼으로 분리된 값들을 공통칼럼을 선정하고 데이터를 통합하는 것을 보편적으로 제3정규화가 되었다.
...
-- PIVOT으로 데이터 펼쳐보기
-- 매입 매출단가가 단가(io_price) 칼럼에 값이 저장되어 있다.
-- 이 값은 io_inout 칼럼의 값을 모름녀 매입인지 매출인지 구별이 어렵다.
-- 따라서 매번 io_inout 칼럼의 값을 표시해야만 매입인지 매출인지 구별이 된다.
-- 그러한 불편을 막기위해 in_out 칼럼의  값을 조건으로 하는 DESECODE 함수를 사용하여
-- 단가부분을 매입단가, 매출단가로 분리하여 보여주도록 Query를 작성하엿다
-- 이제 io_inout 칼럼의 값을 몰라도 현재 데이터가 매입데이터인지, 매출데이터인지 알기 쉽게 되었다.

SELECT io_dname, io_qty,
    DECODE(io_inout, '매입', io_price, 0) AS 매입단가,
    DECODE(io_inout, '매출', io_price, 0) AS 매입단가,
...
-- 거래처 별로 매입과 매출을 구분하여 볼수 있는 쿼리를 완성했다.
-- 거래처별로 매입과 매출의 합계를 계산해보고 싶다.
-- 이 데이터(일년간 거래내역)에서 거래별로 총 매입금액, 총 매출금액을 계산해보고싶다.

-- 1.  '거래처별로' 거래내역이 여러번 반복 이뤄 졌으므로 거래처명이 여러번 나타날 것이다.
-- 이 데이터를 거래별로 묶어주어야한다.
--  GROUP BY io_dname
--2.  전체 매출금액과 매입금액을 표시하는 가상칼럼(매출금액, 매입금액을 계산함)을
-- 통계함수로 묶어준다.
...
-- 00 회사에서 2019년 1년동안 총 매입, 총 매출, 총 이익이 얼마나 발생했는지 알고 싶다.

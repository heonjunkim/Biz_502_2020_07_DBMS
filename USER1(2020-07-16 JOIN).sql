...
-- PK로 설정된 칼럼은 정말 천재지변으로 문제가 발생하지 않는한 저장된 값은
--          변경, 삭제를 금해야 한다.

-- 필요없는 데이터가 발생을 하는 경우 데이터를 삭제하는 대신
-- 특정한 칼럼을 하나 마련하고 , 그 칼럼에 데이터의 사용유무를 등록하여
--      관리하는 것이 실무상 원칙이다.

-- 변경, 삭제 이상을 방지 : 특별한 경우가 아니면 2개 이상의 Record에 변화가 
--                         발생하지 않도록 코드나 관리를 설계 해야 한다.

-- 어떤 테이블에 변경될 가능성이 1이라도 존재하는 칼럼이 있는데
-- 여기에 동일한 값들이 중복되어서 저장되어 있을때 
-- 이 칼럼의 값은 여러 레코드가 변경, 삭제 되는 일이 생길 수 있다.
-- 더불어 이러한 변경, 삭제를 수행 하는 동안 데이터에 문제가 발생할 수 있다.
...
-- 기본 SELECT를 시작
-- FROM 곁에 보고자하는 주 Table(Master)를 작성
-- LEFT JOIN 곁에 보조 정보가 담긴 Table을 작성
-- ON 두테이블의 연관관계를 설정하는 키를 지정
-- SELECT에 필요한 칼럼들을 나열
...
-- JOIN이 된후에 보조테이블의 칼럼을 WHERE 조건설정을하여 SELECTION을 수행할 수 있다.        
SELECT  st_num, st_name, st_dept, d_name,d_prof, d_tel , st_grade, st_tel, st_addr 
FROM tbl_student
    LEFT JOIN tbl_dept
        ON st_dept = d_code
...
-- 원본 Table로 부터 Query를 실행하여 (보통은 임시저장소에 저장해두고)
-- 결과를 보여주는 역할을 한다.
CREATE VIEW view_score
AS (
    SELECT SC .sc_num, ST. st_name,  ST.st_tel,
        ST.st_dept,DT.d_name, DT.d_prof, DT.d_tel, 
        SC.sc_kor, SC.sc_eng, SC.sc_math, sc.sc_music,
        SC.sc_art
    FROM tbl_score SC
...
-- 중요정보를 봐서는 안되는 사용자가 잇을때
-- 보여줘도 문제없는 칼럼만 PROJECTION한 Query를 만들고
-- 이 쿼리를 view로 생성해 두면
-- 권한이 제한된 사용자는 꼭 필요한 정보만을 보게 되어
-- 보안, 개인정보 보호 등의 용도로 사용 할수 있다.

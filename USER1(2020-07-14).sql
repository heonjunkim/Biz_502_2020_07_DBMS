...
-- 물리적인 요소가 많다.
-- CREATE TABLESPACE 저장공간, CREATE USER사용자 정도 이며 schema등을
-- 생성하는 명령 절차
...
-- RDBMS(Relationship Database Management System, 관계형데이터베이스 시스템)에서
-- 데이터를 추가하려면, 먼저 데이터저장공간에 대한 정의를 수행해야 한다.
...
-- 학생데이터를 저장하기 위해서 학생정보 테이블을 생성
-- Table 명명 규칙 : 보통 tbl_접두사로 시작한다. snack case 형식으로 이름을 지정

-- table을 생성하는 DDL
CREATE TABLE tbl_student (
    -- 칼럼(Field) 명명규칙 : 보통 table이름을 줄여서 접두사 시작
    --      snack case 형식으로 이름지정
    -- 문자열 칼럼의 TYPE
    -- CHAR, VARCHAR2
...
-- 데이터를 INSERT 할때 혹시 실수로 PK 칼럼에 이미 저장된 데이터를
-- 또 INSERT 하려고 하면 UNIQUE 오류가 발생하면서 데이터를 저장하지 않는다.
-- 이로써 PK칼럼의 값이 중복되는 것을 방지하여 데이터 무결성을 유지한다.
DROP TABLE tbl_student;
CREATE TABLE tbl_student (
        st_num	CHAR(5)		PRIMARY KEY,
...
SELECT *
FROM tbl_student ;
-- 두개의  칼럼을 나열했는데 VALUE를 1개만 지정하면 INSERT오류 발생
...
-- CRUD에서 Update
-- tbl_student table에 있는 모든데이터(모든행, row, record)의
-- st_dept 칼럼의 값을 컴공과로 변경해라
-- 데이터의 개수에 관계없이 명령이 수행되어 버린다.
-- 아래의 UPDATE 명령은 매우 위험하고 신중하게 사용해야 한다.
UPDATE tbl_student
SET st_dept = '컴공과'; --일주일동안 야근을 해야하는 매우 위험한 방법

-- UPDATE명령어는 특별한 경우가 아니면 항상 PK를기준으로 하라
UPDATE tbl_student
SET st_dept = '무역과'
WHERE st_num = '10004';

-- 학생이름(st_name) 칼럼의 값이 성춘향인 모든 데이터를 찾아서
-- 그 데이터들의 st_dept 칼럼 값을 음악과 로 변경해라
-- 이 명령의 결과로 만약 성춘향 학생의 데이터가 1개밖에 없어서 
-- 원하는 결과를 얻엇더라도, 가급적 사용하지 말아야 하는 명령이다.
-- 어떤 이유로 SELECT 수행했을때 성춘향 데이터가 1개밖에 보이지 않앗더라도
-- 만에 하나 감춰진(보이지않는) 영역에 성춘향 데이터가 또 존재를 한다면
-- 이 명령을 수행하는 순간 이 table의 데이터는 무결성을 잃게 된다.
UPDATE tbl_student 
SET st_dept = '음악과'
WHERE st_name = '성춘향';

-- 학번이 10004, 10007, 10001인 학생의 학과를 물리학 으로 변경하고 싶을때
...
-- 학번이 10002, 10005, 10006 인 학생의 학과를 화학과로 변경하고 싶다.
...
-- 칼럼값을 기준으로 필요한 데이터, 필요없는 데이터를 구분한다.

-- table에서 데이터를 삭제하기 CRUD마지막 Delete명령
-- DELETE명령도 UPDATE 명령과 마찬가지로 반드시 WHERE절을 동반하는 형태로
-- 명령을 수행해야한다.
-- 그렇지 않으면 최소 1주일간 지옥을 맛본다....  :(
DELETE FROM tbl_student
WHERE st_num = '10004';

-- INSERT, UPDATE, DELETE 명령을 수행한후 명령을 확정짓는 명령
-- COMMIT이후에는 ROLLBACK으로 해당 명령을 취소할수 없다.
COMMIT;
...
-- SELECTION 타입으로 테이블 보기
SELECT * FROM tbl_student;

-- 데이터를 Table형식으로 보여줄때 나열된 칼럼들만 보여달라
...
-- st-name + "는" + st_dept + "학과학생입니다"
SELECT st_name ||'는'|| st_dept || '학과학생입니다'
FROM tbl_student
WHERE st_dept = '음악과';
...
-- LIKE 연산자 
-- 문자열 칼럼에서 특정한 문자를 포함한 검색 기능을 구현할때 사용하는 연산자
SELECT *
FROM tbl_student
...
-- 학과가 컴퓨터공학과 이고(AND) 3학년인 학생들만 보여달라
SELECT *
FROM tbl_student
...
-- 학과가 컴퓨터 공학또는 법학인 학생들의 정보중 이름과 학과만 나열하라
...
-- 부등호(범위연산자)를 포함하여 값을 조회할 수 있다.
-- 일반 프로그래밍 코드에서는 문자일 경우는 사용 불가
SELECT st_num, st_name, st_dept, st_grade
FROM tbl_student
WHERE st_num >= '20010' AND st_num <= '20020';

-- 칼럼에 날짜가 문자열로 저장되어있을 경우
-- 날짜 범위를 설정하여 데이터를 조회할수 있다.
-- WHERE st_num >= '2020-01-01' AND st_num <= '2020-12-01';


-- 시작값과 종료값이포함된 범위를 조회하는 코드는

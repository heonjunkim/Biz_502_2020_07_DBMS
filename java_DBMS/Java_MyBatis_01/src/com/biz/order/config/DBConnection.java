...
public class DBConnection {
	...
	static {
		...
		try {
			...
			// SqlSessionFactoryBuilder의 build() method에 Resources.get....(context)
			// 로 읽어들인 inputStream 객체를 매개변수로만 전달을 하면
			// sqlSessionFactory를 만들어서 return 해준다.
			
			// sqlSessionFactory는 프로젝트가 시작될때 만들어져 JVM이 관리하는 객체가 된다.
			// 객체가 되는 이유는(static 이기 때문에)
			// 만약 이 프로젝트를 어떤 경로로 2번이상 작동을 시킨다면
			// 작동되는 횟수만큼 sqlSessionFactory가 계속해서 생성될 것이다.
			...
			// 해주는 것이 좋다.
			// if(sqlSessionFacroty == null) : 아직 아무도 sqlSessionFactory를 만들지 않앗을때만
			// 생성하도록 조건을 부여하는 것이다.
			...
			System.out.println("sqlSessionFactory 생성중 xml 파일문제 발생!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
			// e.printStackTrace();
		}
	}

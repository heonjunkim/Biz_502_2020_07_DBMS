...
public class OrderInput {
	...
	public boolean orderUpdate() {
		...
		System.out.println("수정할 SEQ(QUIT : 중단) >> ");
		...
		try {
			...
			System.out.println("SEQ는 숫자로만 입력하새요");
			return true;
		}
		...
		// orderVO : DB에서 가져온 데이터 1개의 주문서가 담겨있다.
		boolean ret = this.orderInput(orderVO);
	...
	public boolean orderInput(OrderVO orderVO) {
		...
		// orderVO의 각 필드 변수들을 값을 가지고 있을 것이고
		// orderInsert() method 에서 호출했으면
		// orderVO의 각 필드 변수값은 초기값 ""(String)이거나 0(int, long)
		...
		/*
		 * 만약 INSERT 상태일경우 사용자가 주문번호를 입력하지 않으면 setO_num() method를 통과하고 아래쪽 isEmpty()애서
		 * 유효성 검사에서 통과하지 못할 것이다.
		 * 
		 * update 상태일 경우 입력받은 strNum는 isEmpty일 것이고 당연히 setO_num() method는 통과 해버릴 것이다.
		 * 하지만 이미 DB에서 읽어온 값이 담겨있으므로 아래쪽 orderVO.getO_num().isEmpty()는 그냥 통과 될 것이다.
		 */
		if (!strNum.isEmpty()) {
		...
		while (true) {
			...
			if (orderVO.getO_cnum().isEmpty()) {
				System.out.println("고객정보는 반드시 입력!!!!!");
				continue;
			}
			break;
		}
		...
		while (true) {
			System.out.printf("상품가격(%d, QUIT : 중단) >> ", orderVO.getO_pname());
			...
			try {
				...
				System.out.println("상품가격은 숫자만 입력하세요");
			...
			if (orderVO.getO_price() < 1) {
				System.out.println("상품가격0이하 하지마세요");
				continue;
			}
			break;
		}
		...
		while (true) {
			...
			try {
				...
				System.out.println("주문수량은 숫자만 입력하세요");
...
//		} else {
//			System.out.printf("주문서를 %s 하지 못햇습니다", msg);

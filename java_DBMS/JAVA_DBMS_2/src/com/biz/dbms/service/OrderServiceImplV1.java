...
public class OrderServiceImplV1 implements OrderService {
	...
	/*
	 * throws Exception try-catch 를 사용하여 자체적으로 Exception을 HandLing(처리) 하였는데
	 * try-catch가 코드내에 많아지면 코드의 가독성이 매우 떨어진다.
	 * 
	 * Exception 처리를 직접하지 않고 이 method를 호출한 곳에 대리 요청한다(Exception을 던진다라는 표현)
	 */

	/*
	 * insert를 호출하는 곳에서는 order vo에 주문정보 데이터를 담고 매개변수로 전달해야 하며 try-catch 로
	 * exception처리를 하고 결과값으로 정수값을 받아서 insert 유무를 확인해야한다.
	 */
	public int insert(OrderVO orderVO) throws SQLException {
		...
		pSt.setNString(2, orderVO.getPo_date());

		pSt.setNString(4, orderVO.getO_cnum());
	 ...
	 * return 값으로 List(OrderVO) 형 변수에 받아서 처리한다.
	 */
	...
	/*
	 * SelectALL()과 findByseq() method에서 ResultSet으로 부터 데이터를 getter하여 OrderVO에서
	 * setter 하는 부분을 별도 method로 분리하고 selectAll()과 findByseq()에서 호출하여 사용할수 있도록 구현
	 */
	...
	private OrderVO setOrderVO(ResultSet rSet) throws SQLException {
		...
		orderVO.setPo_date(rSet.getString(DBContract.ORDER.COL_PO_DATE_STR.trim()));
	...
	public int update(OrderVO orderVO) throws SQLException {
		...
		pSt.setString(2, orderVO.getPo_date());
